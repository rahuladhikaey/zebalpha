...
export class ManualAdminFSSAIProvider implements FSSAIProvider {
  ...
  async verifyLicense(licenseNumber: string, businessName: string) {
    // Current manual mode: set to Pending Verification for Admin review
    return {
...
export class OfficialFSSAIAPIProvider implements FSSAIProvider {
  ...
  async verifyLicense(licenseNumber: string, businessName: string) {
    // Future integration placeholder for automated FSSAI Portal API lookups
    return {
      status: 'Pending Verification' as const,
...
// Active provider instance (defaults to Manual Admin verification)
