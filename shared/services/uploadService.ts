...
export async function uploadToSupabaseBucket(
  ...
  try {
    ...
    if (error) {
      ...
      // Fallback: get public URL or return base64 if bucket upload fails
...
export async function uploadToCloudinary(
  ...
  try {
    if (typeof fileOrBase64 === "string" && (fileOrBase64.startsWith("http://") || fileOrBase64.startsWith("https://"))) {
      // If URL is already on Cloudinary or web, return directly
      return fileOrBase64;
    }
    ...
    // Fallback: Upload to Supabase bucket 'product-images' if Cloudinary preset is unconfigured
