...
-- ====================================================================
-- 7. ROW-LEVEL SECURITY (RLS) SECURITY HARDENING WITH EXPLICIT ::text CASTS
-- ====================================================================
...
-- B. USER PROFILE POLICIES (STRICT OWNERSHIP WITH ::text CASTS)
CREATE POLICY "Users Read Own Profile" ON public.profiles FOR SELECT USING (auth.uid()::text = id::text OR auth.role() = 'service_role');
CREATE POLICY "Users Update Own Profile" ON public.profiles FOR UPDATE USING (auth.uid()::text = id::text) WITH CHECK (auth.uid()::text = id::text);

-- C. USER ADDRESSES POLICIES (STRICT OWNERSHIP WITH ::text CASTS)
CREATE POLICY "Users Read Own Addresses" ON public.user_addresses FOR SELECT USING (auth.uid()::text = user_id::text OR auth.role() = 'service_role');
CREATE POLICY "Users Insert Own Addresses" ON public.user_addresses FOR INSERT WITH CHECK (auth.uid()::text = user_id::text);
CREATE POLICY "Users Update Own Addresses" ON public.user_addresses FOR UPDATE USING (auth.uid()::text = user_id::text) WITH CHECK (auth.uid()::text = user_id::text);
CREATE POLICY "Users Delete Own Addresses" ON public.user_addresses FOR DELETE USING (auth.uid()::text = user_id::text);

-- D. CART POLICIES (STRICT OWNERSHIP WITH ::text CASTS)
CREATE POLICY "Users Manage Own Cart" ON public.cart_items FOR ALL USING (auth.uid()::text = user_id::text OR auth.role() = 'service_role');

-- E. ORDERS POLICIES
CREATE POLICY "Anyone Can Insert Orders" ON public.orders FOR INSERT WITH CHECK (true);
CREATE POLICY "Users Read Own Orders" ON public.orders FOR SELECT USING (auth.uid()::text = user_id::text OR auth.role() = 'service_role');
...
CREATE POLICY "Users Read Own Card Applications" ON public.card_applications FOR SELECT USING (auth.uid()::text = user_id::text OR auth.role() = 'service_role');
CREATE POLICY "Public Submit Notify Request" ON public.notify_requests FOR INSERT WITH CHECK (true);
CREATE POLICY "Users Read Own Notifications" ON public.notifications FOR SELECT USING (auth.uid()::text = user_id::text OR auth.role() = 'service_role');
